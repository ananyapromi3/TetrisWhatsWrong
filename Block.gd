...
func inactivate_it():
	if is_active:
		...
		is_active = false
		get_tree().root.get_node("GUI").active_block = false
...
func can_move_down():
	if Globals.inactive.has(Vector2(get_parent().position.x + position.x, get_parent().position.y + position.y + 50)) or get_parent().position.y + position.y == 750:
		inactivate_it()
		return false
	else:
		return true
...
func can_move_left():
	if get_parent().position.x + position.x == 0 or (Globals.inactive.has(Vector2(get_parent().position.x + position.x - 50, get_parent().position.y + position.y))) or not is_active:
		return false
	else:
		return true

func can_move_right():
	if get_parent().position.x + position.x == 750 or (Globals.inactive.has(Vector2(get_parent().position.x + position.x + 50, get_parent().position.y + position.y))) or not is_active:
		return false
	else:
		return true
...
func shift_blocks(blocks):
	for i in blocks:
		Globals.inactive[i].y += 50
		Globals.inactive_blocks[i].position.y += 50
